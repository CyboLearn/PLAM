...
import { verifyMagicToken } from "@/actions/auth/verify-magic-token";
...
// UI
import { Field, FieldGroup, Fieldset, Legend } from "@/components/ui/fieldset";
					...
					toast.promise(
						new Promise((resolve, reject) => {
							...
							setTimeout(async () => {
								...
								const { error, message } = await verifyMagicToken(email, otp);
										...
										<>
											<div className="flex">
												{slots.slice(0, 3).map((slot, idx) => (
													// biome-ignore lint/suspicious/noArrayIndexKey: safe to ignore
	...
	return (
		<div
			className={clsx(
				...
				{
					"outline-4 outline-zinc-200/50 dark:outline-zinc-800/50":
						props.isActive,
				},
			)}
		>
